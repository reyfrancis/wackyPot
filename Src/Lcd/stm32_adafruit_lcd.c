...
void BSP_LCD_DisplayStringAt(uint16_t Xpos, uint16_t Ypos, uint8_t *Text,
	...
	while ((*Text != 0)
			& (((BSP_LCD_GetXSize() - (i * DrawProp.pFont->Width)) & 0xFFFF)
 ...
 * @retval None
 */
void BSP_LCD_FillEllipse(uint16_t Xpos, uint16_t Ypos, uint16_t XRadius,
		uint16_t YRadius) {
	int x = 0, y = -YRadius, err = 2 - 2 * XRadius, e2;
	float K = 0, rad1 = 0, rad2 = 0;
 ...
 * @retval None
 */
void BSP_LCD_FillTriangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
	...
	int16_t dx12 = x2 - x1, dy12 = y2 - y1, dx13 = x3 - x1, dy13 = y3 - y1,
...
void BSP_LCD_DrawRGB16Image(uint16_t Xpos, uint16_t Ypos, uint16_t Xsize,
		uint16_t Ysize, uint16_t *pData) {
	lcd_drv->DrawRGBImage(Xpos, Ypos, Xsize, Ysize, (uint8_t*) pData);
}

void BSP_LCD_ReadRGB16Image(uint16_t Xpos, uint16_t Ypos, uint16_t Xsize,
		uint16_t Ysize, uint16_t *pData) {
