''' 
author: zst123
modified: Rey Francis Famulagan 

line 81:
From: pixels = map(lambda x: im.getpixel((x, y))[0], x_coordinates)
To: pixels = map(lambda x: im.getpixel((x, (y_size  - 1)- y))[0], x_coordinates)

This will reverse the Font bitmap generated from bottom to top. 
'''

# Github repo: https://github.com/zst-embedded/STM32-LCD_Font_Generator
# Example usage: python stm32-font.py --font Menlo.ttc --size 32
...
def generate_font_data(font, x_size, y_size):
    ...
    for i, ch in enumerate(get_charset_perceived()):
        ...
        for y in range(y_size):
            ...
            pixels = map(lambda x: im.getpixel((x, (y_size  - 1)- y))[0], x_coordinates)
            # convert to bin text
